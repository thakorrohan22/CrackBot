...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(event.chat_id, f"**I shall be Going Offline!** __because ~ {reason}__")
        else:
            await borg.send_message(event.chat_id, f"**I am Going Offline!**")
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"__My Master Has Been Gone For__ `{total_afk_time}`\nWhere He Is: ~~TERE KO KYU BTAU?😑~~ " + \
            f"\n\n__If you are CIARA then go to pm directly.\nI promise I'll back in a few light years__\n**REASON**: {reason}" \
            if reason \
